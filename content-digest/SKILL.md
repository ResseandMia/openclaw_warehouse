...
使用 feishu-integration skill 保存到飞书：

```bash
cd /root/.openclaw/workspace/skills/feishu-integration/scripts
python3 save_to_feishu.py \
  --title "# MMDD：嘉宾 X 栏目：一句话观点" \
  --content "$(cat /path/to/digest.md)" \
